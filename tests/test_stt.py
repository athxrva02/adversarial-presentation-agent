...
@pytest.mark.skipif(
    ...
    reason="Fixture audio file not found at tests/fixtures/sample.wav",
)
def test_transcribe_real_audio():
    ...
    # We don't assert exact content because Whisper output varies,
    # but it should be non-empty for a speech sample.
    assert len(result) > 0
